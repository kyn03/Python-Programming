# 게임 중 공의 색깔이 계속 랜덤하게 바뀌는 기능 추가

from tkinter import *
import random
import time

tk = Tk()
tk.title("Game")
tk.resizable(0, 0)

canvas = Canvas(tk, width=500, height=400, bd=0, highlightthickness=0)
canvas.pack()

score = 0
score_text = None
game_over_text = None

# 공 색상 후보 리스트
BALL_COLORS = ['red', 'orange', 'yellow', 'green', 'blue', 'purple']

class Ball:
    def __init__(self, canvas, paddle, color):
        self.canvas = canvas
        self.paddle = paddle
        self.color = color

        self.id = canvas.create_oval(10, 10, 25, 25, fill=color)
        self.canvas.move(self.id, 245, 100)

        starts = [-3, -2, -1, 1, 2, 3]
        random.shuffle(starts)
        self.x = starts[0]
        self.y = -3

        self.canvas_height = canvas.winfo_height()
        self.canvas_width = canvas.winfo_width()

        self.hit_bottom = False

        # 색깔 변경을 위한 카운터
        self.color_count = 0

    def hit_paddle(self, pos):
        paddle_pos = self.canvas.coords(self.paddle.id)
        return(
            pos[2] >= paddle_pos[0]
            and pos[0] <= paddle_pos[2]
            and pos[3] >= paddle_pos[1]
            and pos[3] <= paddle_pos[3]
        )

    def change_color(self):
        # 공 색상을 랜덤으로 변경
        new_color = random.choice(BALL_COLORS)
        self.canvas.itemconfig(self.id, fill=new_color)

    def draw(self):
        global score

        if self.hit_bottom:
            return
        
        self.canvas.move(self.id, self.x, self.y)
        pos = self.canvas.coords(self.id)

        # 색 변경 카운트 증가
        self.color_count += 1

        # 일정 횟수마다 공 색상 변경
        if self.color_count % 50 == 0:
            self.change_color()

        if pos[1] <= 0:
            self.y = 3
        
        if pos[3] >= self.canvas_height:
            self.hit_bottom = True
        
        if self.hit_paddle(pos):
            self.y = -3
            score += 10
            update_score()

        if pos[0] <= 0:
            self.x = 3

        if pos[2] >= self.canvas_width:
            self.x = -3

class Paddle:
    def __init__(self, canvas, color):
        self.canvas = canvas
        self.id = canvas.create_rectangle(0, 0, 100, 10, fill=color)
        self.canvas.move(self.id, 200, 300)

        self.x = 0
        self.canvas_width = canvas.winfo_width()

        canvas.bind_all('<KeyPress-Left>', self.turn_left)
        canvas.bind_all('<KeyPress-Right>', self.turn_right)

    def turn_left(self, evt):
        self.x = -3

    def turn_right(self, evt):
        self.x = 3

    def draw(self):
        self.canvas.move(self.id, self.x, 0)
        pos = self.canvas.coords(self.id)
        if pos[0] <= 0 or pos[2] >= self.canvas_width:
            self.x = 0

def update_score():
    global score_text
    canvas.itemconfig(score_text, text=f"Score: {score}")

def restart_game():
    global ball, paddle, score, score_text, game_over_text
    canvas.delete("all")
    score = 0
    score_text = canvas.create_text(
        50, 20,
        text="Score: 0",
        fill="black",
        font=("Arial", 14),
        anchor="w"
    )

    paddle = Paddle(canvas, 'blue')
    ball = Ball(canvas, paddle, random.choice(BALL_COLORS))
    game_over_text = None

score_text = canvas.create_text(
    50, 20,
    text="Score: 0",
    fill="black",
    font=("Arial", 14),
    anchor="w"
)

paddle = Paddle(canvas, 'blue')
ball = Ball(canvas, paddle, random.choice(BALL_COLORS))

restart_btn = Button(tk, text="Restart", command=restart_game)
restart_btn.pack(pady=5)

while True:
    if not ball.hit_bottom:
        ball.draw()
        paddle.draw()

    else:
        if game_over_text is None:
            game_over_text = canvas.create_text(
                250, 200,
                text="GAME OVER",
                font=("Arial", 30),
                fill = "red"
            )
    tk.update()
    time.sleep(0.01)